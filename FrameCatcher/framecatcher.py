...
class FrameCatcher(object):
    """A class of support functions to be paired with the FrameCatcher C++
    plugin to enable render-time frame inspection."""
    ...
    def _bootstrap(self, debug=False):
        """Completes initialisation by creating dirs and padding node variables.
        ...
        if self._enable:
            ...
            logdir = os.path.join(os.path.dirname(self._context['file'].evaluate()),
                                  FC_LOG_SUBDIR)
            ...
            # now stick in a whole bunch of nodes with the right boot string
            self._catchers = []
    ...
    def before_frame(self):
        ...
        if self._enable:
            ...
            for fc in self._catchers:
                ...
                fc['disable'].setValue(False)
                fc['clock'].setValue(not fc['clock'].value())
                # this single line slows down everything by invalidating the
                # cache from the read node on each frame - this will report
                # more accurately multiframe blends and it also seems to make
                # things er. Work.
                # I clearly don't 100% get the threading/multiproc model
    ...
    def after_frame(self):
        ...
        if self._enable:
            ...
            for x in candidates:
                ...
                # If this is called within the with/for clause loop we get file
                # locking issues. Basically, the file unlocking is not happing
                # fast enough unless you jump out the second with clause.
                # This bites.
...
def find_catchers():
    """Convenience function to print fullname of all FrameCatcher instances."""
