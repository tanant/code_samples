...
/*
0.91 - additional inclusion of a convenience field/header to help with the compacting process
0.9d - minor tweak to the guard positioning in _open, trying to minimise the time we could be
       having clashes
0.9c - added in two FuryFX conveniences - the file and frame fragments.
0.9b - help text refined, and the default outfile set to be more sane (so it Just Works)
0.9a - first trial release
*/

static const char* const AUTHOR = "anthony.tan@greenworm.net";
