...
class DeliverablePackage(object):
    ...
    def name(self):
        return "{seq}_{shot}_v{major}_{minor}".format(seq=self.seq.upper(),
                                                      shot=self.shot.upper(),
                                                      major=self.major,
                                                      minor=self.minor)
...
class DIDeliverable(DeliverablePackage):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.base = os.path.join(
            self.root, self.type, "{seq}_{shot}_v{major}_{minor}".format(**self.__dict__))
        newbase = os.path.join(
            '{base}', '{seq}_{shot}_v{major}').format(**self.__dict__)
        try:
        ...
        try:
            ...
            components = [x for x in components if not x.lower().startswith('.') and
                          os.path.isdir(os.path.join(newbase, x))]

        except Exception:
            raise ValueError(
                "internal directory doesn't match major/minor wrapper. Have you manually renamed stuff?")
    ...
    def get_mov(self):
        newbase = os.path.join('{base}',
                               '{seq}_{shot}_v{major}',
                               '{uid}').format(**self.__dict__)
        newbase = path_tools.sensitise_to_case(newbase)
        try:
    ...
    def get_frame_base(self):
        newbase = os.path.join('{base}',
                               '{seq}_{shot}_v{major}',
                               '{resolution}').format(**self.__dict__)
        newbase = path_tools.sensitise_to_case(newbase)
...
class DIDeliverable_legacy(DeliverablePackage):
    ...
    def __init__(self, uid=None, *args, **kwargs):
        ...
        super(DIDeliverable_legacy, self).__init__(major='999',
                                                   minor='999',
                                                   *args,
                                                   **kwargs)
        self.uid = uid
        self.base = os.path.join(
            self.root, self.type, "{uid}".format(**self.__dict__))
...
class EditorialDeliverable(DeliverablePackage):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.type = 'editorial'
        self.base = os.path.join(self.root,
                                 self.type,
                                 "{seq}_{shot}_v{major}_{minor}".format(**self.__dict__))
