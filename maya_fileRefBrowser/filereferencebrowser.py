...
class FileReferenceBrowser(QtGui.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.uifile = r'.\resources\ref_ed.ui'
        ...
        self.UI.togglerenderthumbButton.clicked.connect(
            tt_mt.toggle_renderthumbs)
    ...
    def repathPenButton_clicked(self):
        ...
        root_replacement_pairs = [(r'(?i)H:/shots/{seq}/{seq}_{shot}',
                                   r'N:/DeliveryHolding/deliveriesOut/delivery_prep/{seq}_{shot}'),

                                  (r'(?i)//vfx/fury/shots/{seq}/{seq}_{shot}',
                                   r'N:/DeliveryHolding/deliveriesOut/delivery_prep/{seq}_{shot}'),
                                  ]
        ...
        # first thing before the colon
        node_attrib_name = source_file.split(':')[0]
        ...
        for pair in root_replacement_pairs:
            ...
            if re.search(pair[0].format(shot=shot, seq=seq), source_file):
                ...
                # new string
                new_text = ':'.join(re.sub(pair[0].format(shot=shot, seq=seq),
                                           pair[1].format(shot=shot, seq=seq),
                                           source_file).split(':')[1:])
                print new_text
                pm.PyNode(str(current_item.text(0))).attr(
                    node_attrib_name).set(new_text)
    ...
    def localiseButton_clicked(self):
        ...
        for x in self.UI.nodeTree.selectedItems():
            ...
            # assuming we don't have too many colons..
            file_name = os.path.basename(source_file)
            target_dir = os.path.join(self.holding_pen.format(shot=shot, seq=seq),
                                      "components",
                                      "textures",
                                      '.'.join(os.path.basename(source_file).split('.')[0:-1]))
            file_sourcedir = os.path.dirname(
                ':'.join(source_file.split(':')[1:]))
            # first thing before the colon
            node_attrib_name = source_file.split(':')[0]
            ...
            if str(self.UI.nodeTree.itemWidget(current_item, 2)) == 'sequence':
                ...
                try:
                    shutil.copy(
                        os.path.join(file_sourcedir, file_name), target_dir)
                except:
                    pass

            pm.PyNode(str(current_item.text(0))).attr(
                node_attrib_name).set(os.path.join(target_dir, file_name))
    ...
    def populate_all(self):
        ...
        # updated regexp to match EARLY in the seqshot, so we don't
        # get faked by people with a scene from elsewhere.
    ...
    def populate_cachefile(self):
        ...
        for x in cachefile:
            # create a new widget under the cat head, parenting (puts in the
            # right area)
            new_widget = QtGui.QTreeWidgetItem(category_head)
            # give it a name in column zero - this is text
            new_widget.setText(0, x.name())
            try:
                # create a line_edit item, which holds text
                new_line_edit = QtGui.QLabel(
                    'cachePath:' + x.attr('cachePath').get())
            except:
                # create a line_edit item, which holds text
                new_line_edit = QtGui.QLabel('cachePath:UNKNOWN')
            # and the bind the line edit into the table for col 1, and
            # whereever new_widget is
            self.UI.nodeTree.setItemWidget(new_widget, 1, new_line_edit)
        ...
        for x in cachefile:
            ...
            try:
                # create a line_edit item, which holds text
                new_line_edit = QtGui.QLabel(
                    'cacheDirectory:' + x.attr('cacheDirectory').get())
            except:
                # create a line_edit item, which holds text
                new_line_edit = QtGui.QLabel('cacheDirectory:UNKNOWN')
            # and the bind the line edit into the table for col 1, and
            # whereever new_widget is
            self.UI.nodeTree.setItemWidget(new_widget, 1, new_line_edit)
    ...
    def populate_filetex(self):
        ...
        for x in filetex:
            ...
            # create a line_edit item, which holds text
            new_line_edit = QtGui.QLabel(
                'fileTextureName:' + x.attr('fileTextureName').get())
    ...
    def populate_implanes(self):
        ...
        for x in filetex:
            ...
            # create a line_edit item, which holds text
            new_line_edit = QtGui.QLabel(
                'imageName:' + x.attr('imageName').get())
