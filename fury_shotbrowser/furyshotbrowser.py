...
class FuryShotBrowser(QtGui.QWidget):
    ...
    def __init__(self, shotManager):
        ...
        # load interface
        self.browser_widget = uic.loadUi(
            os.path.join(FuryTools.__pkgroot__, 'resources', 'TT_FuryShotBrowser.ui'))
        ...
        self.browser_widget.seqWidget.currentItemChanged.connect(
            self.seq_updated)
        self.browser_widget.shotWidget.currentItemChanged.connect(
            self.shot_updated)
        self.browser_widget.userWidget.currentIndexChanged.connect(
            self.user_updated)

        self.browser_widget.fileWidget.doubleClicked.connect(
            self.doubleClick_file)
        self.browser_widget.otherFiles.doubleClicked.connect(
            self.doubleClick_otheruser)
        self.browser_widget.shotWidget.doubleClicked.connect(
            self.set_project)
        ...
        self.browser_widget.refreshButton.clicked.connect(
            self.doubleClick_refresh)

        self.browser_widget.filepathWidget.setReadOnly(True)
        self.setWindowTitle("Fury Maya Shot Browser v1.02")

        self.setGeometry(self.geometry().left() + 50,
                         self.geometry().top() + 50,
                         self.browser_widget.geometry().width(),
                         self.browser_widget.geometry().height())
        self.browser_widget.setGeometry(0, 0, self.width(), self.height())

        # you need to set the context (i.e. right click) menu policy to fire
        # the custom signal

        self.browser_widget.shotWidget.setContextMenuPolicy(
            Qt.CustomContextMenu)
        self.browser_widget.shotWidget.customContextMenuRequested.connect(
            self.contextclick_pkg)
        self.browser_widget.fileWidget.setContextMenuPolicy(
            Qt.CustomContextMenu)
        self.browser_widget.fileWidget.customContextMenuRequested.connect(
            self.contextclick_pkg)
        self.browser_widget.seqWidget.setContextMenuPolicy(
            Qt.CustomContextMenu)
        self.browser_widget.seqWidget.customContextMenuRequested.connect(
            self.contextclick_pkg)
    ...
    def set_project(self, *args, **kwargs):
        try:
            ...
            FTmaya.setProject(
                self.shotManager.mayaprojectpathFromSeqShot(shot, seq, 'maya', user))
        except:
            pass
    ...
    def contextclick_handler(self, *args, **kwargs):
        ...
        if sender == self.showBrowser:
            ...
            try:
                ...
                # we can handle this without dying now
                pass
    ...
    def updateUserList(self, *args, **kwargs):
        ...
        self.browser_widget.userWidget.addItems(
            sorted([value for key, value in self.machineUserMap.items()]))

        self.browser_widget.userWidget.setCurrentIndex(
            self.browser_widget.userWidget.findText(self.shotManager.mapMachineToUser()))
    ...
    def updateSequence(self, *args, **kwargs):
        ...
        self.browser_widget.seqWidget.addItems(
            sorted([key for key, value in self.seqShotDict.items()]))
    ...
    def doubleClick_otheruser(self, *args, **kwargs):
        ...
        shot = str(self.browser_widget.shotWidget.currentItem().text())
        filename = str(
            self.browser_widget.otherFiles.currentItem().text().split(':')[1])
        user = str(
            self.browser_widget.otherFiles.currentItem().text().split(':')[0])
    ...
    def open_file(self, seq, shot, filename, user):
        if filename == self.NEWFILE:
            ...
            result = FTmaya.loadFile(os.path.join(
                self.shotManager.softwarepathFromSeqShot(shot, seq, 'maya', user), filename))
    ...
    def user_updated(self, *args, **kwargs):
        if args[0] is None:
            ...
            # this is a rare case for a QComboBox. Might actually not be
            # possible?
            pass
        else:
    ...
    def seq_updated(self, *args, **kwargs):
        try:
            ...
            pass    # all other errors are fatal
    ...
    def shotcode_changed(self):
        try:
            ...
            shotfiles = self.shotManager.getShotFilesForAll(
                shot=shot, seq=seq, software='maya', stripEmpties=True)
            ...
            self.browser_widget.otherFiles.addItems(
                self.shotManager.getShotVersionsForAll(shot=shot, seq=seq, software='maya'))

            self.browser_widget.filepathWidget.setText(
                self.shotManager.softwarepathFromSeqShot(seq, shot, 'maya', usr))
        except AttributeError:
            pass

    def populate_shots_from_seq(self, seq_code):
        try:
            # QT uses unicode, and we're using unicode as labels. This isn't
